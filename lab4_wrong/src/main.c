...
//start of code from example project
...
__IO uint16_t TIM3_CCR1_Val = 250;//should we initialize to something else? 
...
int main(void){
     ...
     */
		...
		TIM3_PWM_Config();//how to configure 
...
{																																//for timer4 
		/*if ((*htim).Instance==TIM4) {
			//retrieve the ADC converted value for every TIM4
			//multiply it by the weird coefficient
			//set it to measured temp
			if (measuredTemp >= setPoint) {
			//now, if it equals set point, we want to be at a CCR of 150 which is a TWENTY PERCENT DUTY 
			//if measured temp is double set point, we want a CCR of 1000 --100% duty
			//therefore, we can take the value of measuredtemp/setpoint and subtract setpoint, then multiply by 1000
